...
def install_xiaobai(d):
    if d.shell("test -d /data/local/tmp/apk && echo 'exist' || echo 'does not exist'").output == 'does not exist':
    ...
    while True:
        try:
            if d.info['sdkInt'] <= 29:
                ...
                d.shell('media volume --show --stream 3 --set 0')
            time.sleep(0.3)
        except:
            pass
...
def test_app(d, last_time, package_name, monkey_interval):
    ...
    if d.info['productName'] == 'sagit':
        if package_name == "com.oplus.camera":
            package_name = "com.android.camera"
        if package_name == "com.heytap.browser":
            package_name = "com.android.browser"
    if d.info['productName'] == 'VCE-AL00':
        if package_name == "com.oplus.camera":
            package_name = "com.huawei.camera"
        if package_name == "com.heytap.browser":
            package_name = "com.android.browser"
...
class T_tracing(threading.Thread):
    def __init__(self, d, interval, duration, tracing_script):
        threading.Thread.__init__(self)
        self.d = d
        self.interval = interval
        self.duration = duration
        self.tracing_script = tracing_script

    def run(self):
        self.result = self.tracing_script(self.d, self.interval, self.duration)
